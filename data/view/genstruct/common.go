...
import (
	...
	"github.com/liuchenrang/gormt/data/config"
	"github.com/liuchenrang/gormt/data/view/cnf"
	"github.com/liuchenrang/gormt/data/view/generate"
	"github.com/liuchenrang/gormt/data/view/genfunc"
)
...
func (s *GenStruct) GenerateHookSave(fieldNames []string) []string {
	tmpl, err := template.New("gen_tnf").Parse(genfunc.GetHookSaveNameTemp())
	if err != nil {
		panic(err)
	}
	var data struct {
	...
	if data.U == nil {
		panic("找不到设置的字段 " + strings.Join(fieldNames, ","))
	}
	data.TableName, data.StructName = s.TableName, s.Name
	var buf bytes.Buffer
	tmpl.Execute(&buf, data)
...
func (s *GenStruct) GenerateHookCreate(fieldNames []string) []string {
	tmpl, err := template.New("gen_tnf").Parse(genfunc.GetHookCreateNameTemp())
	if err != nil {
		panic(err)
	}
	var data struct {
	...
	for i := range s.Em {
		for fi := range fieldNames {
			element := s.Em[i]
			if fieldNames[fi] == element.Name {
				data.FS = append(data.FS, &element)
			}
		}
	}
	if data.FS == nil {
...
func (s *GenStruct) Generates() []string {
	...
	for _, v := range s.Em {
		if !mp[v.Name] {
			if config.IsSoftDeleteEnabled() {
				if v.ColumnName == config.GetSoftDeleteFieldName() {
					v.Tags["gorm"] = append(v.Tags["gorm"], "softDelete:flag")
					v.Type = "soft_delete.DeletedAt"
				}
			}
...
func (p *GenPackage) Generate() string {
	...
	// add import
	if p.Imports != nil {
		hasCreate := config.GetHookBeforeCreate() != nil && len(config.GetHookBeforeCreate()) > 0
	...
	for _, v := range p.Structs {
		...
		if config.GetHookBeforeSave() != nil { // add table name func
			save, ok := config.GetHookBeforeSave()["updateTime"]
			if ok {
				fieldNames := strings.Split(save, ",")
				for _, v1 := range v.GenerateHookSave(fieldNames) {
					pa.Add(v1)
				}
			}

		}
		if config.GetHookBeforeCreate() != nil { // add table name func
			save, ok := config.GetHookBeforeCreate()["updateTime"]
			if ok {
				fieldNames := strings.Split(save, ",")
				for _, v1 := range v.GenerateHookCreate(fieldNames) {
					pa.Add(v1)
				}
			}
