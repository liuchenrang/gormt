...
import (
	...
	"github.com/liuchenrang/gormt/data/view/genstruct"
)
	...
	if m.pkg == nil {
		for _, tab := range m.info.TabList {
			...
			if config.IsSoftDeleteEnabled() {
				pkg.AddImport(" \"gorm.io/plugin/soft_delete\"")
			}
			var sct genstruct.GenStruct
			sct.SetStructName(getCamelName(tab.Name)) // Big hump.大驼峰
			sct.SetNotes(tab.Notes)
			...
			if config.IsTableNameUseCamelName() {
				stt.FileName = strings.ToLower(getCamelName(tab.Name)[:1]) + getCamelName(tab.Name)[1:] + ".go"
			} else {
